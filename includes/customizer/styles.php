...
function woocustomizer_customizer_library_build_styles() {
	...
	if ( wcz_fs()->can_use_premium_code__premium_only() ) {
		...
		if ( get_option( 'wcz-enable-stickcart', woocustomizer_library_get_default( 'wcz-enable-stickcart' ) ) && get_option( 'wcz-stickcart-edit', woocustomizer_library_get_default( 'wcz-stickcart-edit' ) ) ) :
			...
			if ( $mod !== woocustomizer_library_get_default( $setting ) ) {
				$wcz_stickcart_title = esc_attr( $mod );
	
				WooCustomizer_Library_Styles()->add( array(
					'selectors' => array(
						'body.single-product .wcz-sticky-addtocart .wcz-sticky-addtocart-title' // Woo Blocks
					),
					'declarations' => array(
						'font-size' => $wcz_stickcart_title . 'px'
					)
				) );
			}
			// Sticky Cart Colors
			$setting = 'wcz-stickcart-bgcolor';
			$mod = get_option( $setting, woocustomizer_library_get_default( $setting ) );
	
			if ( $mod !== woocustomizer_library_get_default( $setting ) ) {
				$wcz_stickcart_bgcolor = wcz_sanitize_hex_color( $mod );
	
				WooCustomizer_Library_Styles()->add( array(
					'selectors' => array(
						'body.single-product .wcz-sticky-addtocart,
						body.single-product .wcz-sticky-addtocart-inner'
					),
					'declarations' => array(
						'background-color' => $wcz_stickcart_bgcolor,
						'color' => wcz_getContrastColor( $wcz_stickcart_bgcolor )
					)
				) );
			}
			$setting = 'wcz-stickcart-fontcolor';
			$mod = get_option( $setting, woocustomizer_library_get_default( $setting ) );
			...
			if ( $mod !== woocustomizer_library_get_default( $setting ) ) {
				...
				WooCustomizer_Library_Styles()->add( array(
					...
					'declarations' => array(
						'color' => $wcz_stickcart_bgcolor
					)
				) );
			}
			// Sticky Cart Full Width
			$setting = 'wcz-stickcart-fullwidth';
			$mod = get_option( $setting, woocustomizer_library_get_default( $setting ) );
	
			if ( $mod !== woocustomizer_library_get_default( $setting ) ) {
				WooCustomizer_Library_Styles()->add( array(
					'selectors' => array(
						'.wcz-sticky-addtocart .wcz-sticky-addtocart-inner'
					),
					'declarations' => array(
						'max-width' => 'none'
					)
			...
			// Remove Star Rating
			$setting = 'wcz-stickcart-remove-onmobile';
			$mod = get_option( $setting, woocustomizer_library_get_default( $setting ) );
			...
			if ( $mod !== woocustomizer_library_get_default( $setting ) ) {
				WooCustomizer_Library_Styles()->add( array(
					...
					'media' => esc_attr( '(max-width: 782px)' )
				) );
			}
	...
	if ( wcz_fs()->can_use_premium_code__premium_only() ) {
		if ( get_option( 'wcz-direct-checkout', woocustomizer_library_get_default( 'wcz-direct-checkout' ) ) ) :
			$setting = 'wcz-directcheckout-spacing';
			$mod = get_option( $setting, woocustomizer_library_get_default( $setting ) );

			if ( $mod !== woocustomizer_library_get_default( $setting ) ) {

				$wcz_directcheckout_space = esc_attr( $mod );

				WooCustomizer_Library_Styles()->add( array(
					'selectors' => array(
						'body.woocommerce-checkout .wcz-direct-checkout-cart'
					),
					'declarations' => array(
						'margin-bottom' => $wcz_directcheckout_space . 'px'
					)
				) );
			}
			if ( get_option( 'wcz-directcheckout-notice', woocustomizer_library_get_default( 'wcz-directcheckout-notice' ) ) ) :
				$setting = 'wcz-directcheckout-notice-color';
				$mod = get_option( $setting, woocustomizer_library_get_default( $setting ) );

				if ( $mod !== woocustomizer_library_get_default( $setting ) ) {

					$wcz_dcnotice_color = wcz_sanitize_hex_color( $mod );
					...
					WooCustomizer_Library_Styles()->add( array(
						...
						'declarations' => array(
							'background-color' => $wcz_dcnotice_color,
							'color' => wcz_getContrastColor( $wcz_dcnotice_color )
						)
					) );
				}
				$setting = 'wcz-directcheckout-notice-fcolor';
				$mod = get_option( $setting, woocustomizer_library_get_default( $setting ) );

				if ( $mod !== woocustomizer_library_get_default( $setting ) ) {

					$wcz_dcnotice_fcolor = wcz_sanitize_hex_color( $mod );
	...
	if ( wcz_fs()->can_use_premium_code__premium_only() ) {
		...
		$setting = 'wcz-qv-link-botmargin';
		$mod = get_option( $setting, woocustomizer_library_get_default( $setting ) );

		if ( $mod !== woocustomizer_library_get_default( $setting ) ) {

			$wczqv_link_botmargin = esc_attr( $mod );
			...
			WooCustomizer_Library_Styles()->add( array(
				...
				'declarations' => array(
					'margin-bottom' => $wczqv_link_botmargin . 'px'
				)
			) );
		}
		$setting = 'wcz-qv-over-bottom';
		$mod = get_option( $setting, woocustomizer_library_get_default( $setting ) );

		if ( $mod !== woocustomizer_library_get_default( $setting ) ) {

			$wczqv_over_botmargin = esc_attr( $mod );
			...
			WooCustomizer_Library_Styles()->add( array(
				...
				'declarations' => array(
					'bottom' => $wczqv_over_botmargin . 'px'
				)
			) );
		}
