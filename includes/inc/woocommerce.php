...
function wcz_wc_texts() {
	...
	if ( is_cart() && get_option( 'wcz-cart-add-product-info', woocustomizer_library_get_default( 'wcz-cart-add-product-info' ) ) ) {
        if ( get_option( 'wcz-cart-add-productinfo-cats', woocustomizer_library_get_default( 'wcz-cart-add-productinfo-cats' ) ) ) {
			add_action( 'woocommerce_after_cart_item_name', 'wcz_add_cart_page_categories', 99, 3);
		}
		if ( get_option( 'wcz-cart-add-productinfo-stock', woocustomizer_library_get_default( 'wcz-cart-add-productinfo-stock' ) ) ) {
			add_action( 'woocommerce_after_cart_item_name', 'wcz_add_cart_page_stock', 99, 2 );
		}
	}

	if ( wcz_fs()->can_use_premium_code__premium_only() ) {
		// Direct Checkout
		if ( ( is_cart() || is_checkout() ) && get_option( 'wcz-direct-checkout', woocustomizer_library_get_default( 'wcz-direct-checkout' ) ) ) {
			wcz_redirect_cart_to_checkout();
		}
		if ( get_option( 'wcz-direct-checkout', woocustomizer_library_get_default( 'wcz-direct-checkout' ) ) && get_option( 'wcz-directcheckout-disable-cartout', woocustomizer_library_get_default( 'wcz-directcheckout-disable-cartout' ) ) && get_option( 'wcz-directcheckout-notice', woocustomizer_library_get_default( 'wcz-directcheckout-notice' ) ) ) {
			add_action( 'wp_body_open', 'wcz_directnotice_notice', 10 );
		}
		if ( is_checkout() && get_option( 'wcz-directcheckout-remlinks', woocustomizer_library_get_default( 'wcz-directcheckout-remlinks' ) ) ) {
...
function wcz_add_cart_page_categories( $cart_item, $cart_item_key ) {
	...
	echo ( $cat_ids ) ? '<div class="wcz-cart-cats">' . wc_get_product_category_list( $product_item->get_id(), ', ', '<span class="posted_in">' . _n( 'Category:', 'Categories:', count( $cat_ids ), 'woocustomizer' ) . ' ', '</span></div>' ) : '';
...
function wcz_add_notice_banner_wc() {
    ...
    <div class="wcz-banner-notice <?php echo get_option( 'wcz-wc-notice-design', woocustomizer_library_get_default( 'wcz-wc-notice-design' ) ) ? sanitize_html_class( $wcz_notice_style ) : sanitize_html_class( 'wcz-notice-one' ); ?>">
        ...
        <p><?php echo wp_kses( $wcz_notice_text , $allowedtags ); ?></p>
    </div><?php
}
...
function wcz_add_admin_stats_btn() {
    if ( current_user_can('manage_options') ) { ?>
        <button class="wcz-adminstats-btn" title="<?php esc_attr_e( 'View Product Statistics', 'woocustomizer' ); ?>" data-productid="<?php echo esc_attr( get_the_ID() ); ?>"></button>
    <?php
    }
}
// Add Footer Modal
...
function wcz_wc_extras() {
	...
	if ( is_checkout() && get_option( 'wcz-checkout-edit-headings', woocustomizer_library_get_default( 'wcz-checkout-edit-headings' ) ) ) {
		add_filter( 'gettext', 'wcz_edit_checkout_page_headings', 20, 3 );
	}
...
function wcz_add_product_long_desc() {
	...
	<div class="wcz-product-long-desc <?php echo $wcz_longdesc_center ? 'center' : ''; ?> <?php echo sanitize_html_class( 'wcz-longdescdiv-' . get_option( 'wcz-longdesc-divider', woocustomizer_library_get_default( 'wcz-longdesc-divider' ) ) ); ?> <?php echo $wcz_longdesc_bd ? 'bot-divider' : ''; ?>">
		<div class="wcz-inner" <?php echo $wcz_longdesc_mwidth ? 'style="max-width: ' . esc_attr( $wcz_longdesc_mwidth ) . 'px;"' : ''; ?>>
...
function wcz_add_product_reviews() {
	...
	<div class="wcz-product-reviews <?php echo sanitize_html_class( 'wcz-reviewsdiv-' . get_option( 'wcz-reviews-divider', woocustomizer_library_get_default( 'wcz-reviews-divider' ) ) ); ?> <?php echo $wcz_reviews_bd ? 'bot-divider' : ''; ?>">
		<div class="wcz-inner" <?php echo $wcz_reviews_mwidth ? 'style="max-width: ' . esc_attr( $wcz_reviews_mwidth ) . 'px;"' : ''; ?>>
...
function wcz_add_product_addinfo() {
	...
	<div class="wcz-product-addinfo <?php echo sanitize_html_class( 'wcz-addinfodiv-' . get_option( 'wcz-addinfo-divider', woocustomizer_library_get_default( 'wcz-addinfo-divider' ) ) ); ?> <?php echo $wcz_addinfo_bd ? 'bot-divider' : ''; ?>">
		<div class="wcz-inner" <?php echo $wcz_addinfo_mwidth ? 'style="max-width: ' . esc_attr( $wcz_addinfo_mwidth ) . 'px;"' : ''; ?>>
	...
	if ( is_account_page() && 'wcz-account-dashboard-edit' == get_option( 'wcz-account-dashboard-tab', woocustomizer_library_get_default( 'wcz-account-dashboard-tab' ) ) && 'default' !== get_option( 'wcz-account-dashboard-content', woocustomizer_library_get_default( 'wcz-account-dashboard-content' ) ) ) {
		...
		echo '<div class="wcz-dash-content" >';
			...
			if ( 'wcz-accdash-content-page' == $wcz_dash_display ) {
				...
				$wcz_dashpage = apply_filters( 'wpml_object_id', $wcz_dashpage_id, 'page' ); // WPML fix for translations
			...
			} else if ( 'wcz-accdash-content-text' == $wcz_dash_display ) {
        ...
        if ( ( isset( $args->menu ) && $args->menu == $wcz_logmenu ) || ( isset( $args->menu->slug ) && $args->menu->slug == $wcz_logmenu ) || ( isset( $args->theme_location ) && $args->theme_location == $wcz_logmenu ) ) {
			...
			ob_start();
				if ( is_user_logged_in() ) : ?>
					<a href="<?php echo esc_url( $wcz_logout_url ); ?>"><?php echo esc_html( $wcz_logout_txt ); ?></a>
				<?php
				else : ?>
...
add_filter( 'woocommerce_login_redirect', 'wcz_customer_login_redirect', 999, 2 );
...
if ( wcz_fs()->can_use_premium_code__premium_only() ) {
	...
	function wcz_redirect_cart_to_checkout() {
		// Default to Checkout page
		$wcz_url = esc_url( wc_get_checkout_url() );

		if ( is_cart() ) {
			if ( get_option( 'wcz-directcheckout-disable-cartout', woocustomizer_library_get_default( 'wcz-directcheckout-disable-cartout' ) ) && 0 == WC()->cart->get_cart_contents_count() ) {
				...
				$wcz_url = get_permalink( $wczredirectid ); // Redirects to Selected Page if checkout is disabled
			}

			// Add notice
			if ( get_option( 'wcz-directcheckout-notice', woocustomizer_library_get_default( 'wcz-directcheckout-notice' ) ) ) {
				$wcz_url .= "?note=1";
			}
		...
		if ( is_checkout() && get_option( 'wcz-directcheckout-disable-cartout', woocustomizer_library_get_default( 'wcz-directcheckout-disable-cartout' ) ) ) {
			if ( 0 == WC()->cart->get_cart_contents_count() && ! is_wc_endpoint_url( 'order-pay' ) && ! is_wc_endpoint_url( 'order-received' ) ) {
				$wczredirectid = get_option( 'wcz-directcheckout-disabled-redirect', woocustomizer_library_get_default( 'wcz-directcheckout-disabled-redirect' ) );
				$wcz_url = get_permalink( $wczredirectid );
				...
				wp_redirect( $wcz_url ); // Redirects to Selected Page
				exit;
			}
		}
	}
	function wcz_directnotice_notice() {
		$noticeset = isset( $_GET['note'] ) ? true : false;
		$notice_redirect = get_option( 'wcz-directcheckout-disabled-redirect', woocustomizer_library_get_default( 'wcz-directcheckout-disabled-redirect' ) );
		$notice_message = get_option( 'wcz-directcheckout-notice-txt', woocustomizer_library_get_default( 'wcz-directcheckout-notice-txt' ) );
		...
		if ( $noticeset && $page_id == $notice_redirect ) {
			echo '<div class="wcz-direct-checkout-notice">' . esc_html( $notice_message ) . '</div>';
		}
	}
	// Add Cart to Checkout page
	function wcz_add_cart_to_checkout() {
		if ( !get_option( 'wcz-directcheckout-addcart', woocustomizer_library_get_default( 'wcz-directcheckout-addcart' ) ) ) return;
		if ( is_wc_endpoint_url( 'order-received' ) ) return;
		echo '<div class="wcz-direct-checkout-cart">' . do_shortcode('[woocommerce_cart]') . '</div>';
	}
	add_action( 'woocommerce_before_checkout_form', 'wcz_add_cart_to_checkout', 5 );
